...
function update() {
    ...
    spatialData = spatialData.filter(row => Number.isFinite(row[otherOptions.measureField]))
...
function getDemographicData(data, dataState, highlight=null) {
    ...
    let demographicData = data.filter(d => d.state_fips == stateFips && d.county_fips == countyFips)
    ...
    SELECTABLE_FIELDS.forEach(field => {
        if (stratifySet.has(field)) {
            ...
            demographicData = demographicData.filter(row => row[field] == dataState[stateName("select", field)])
        }
    })
...
function plotDemographic(data, highlightData=null) {
    const mainField = dataOptionsState.compareSecondary != "none" ? dataOptionsState.compareSecondary : dataOptionsState.comparePrimary
    const otherField = dataOptionsState.compareSecondary != "none" ? dataOptionsState.comparePrimary : "none"
    ...
    const options = {
      ...
      marginTop: 30,
      marginRight: 60,
      y: {grid: true, axis: "right", label: otherOptions.measureField + " ↑", tickFormat: tickFormat},
      x: {domain: domainValues, tickFormat: d => l(d)},
      marks: [
        Plot.barY(highlightData, {x: mainField, y: otherOptions.measureField, fill: d => colorScheme(d[mainField]), facet: true}),
        Plot.tickY(data, {x: mainField, y: otherOptions.measureField, strokeWidth: 2, strokeDasharray: "3,2"}),
      ]
    }
    ...
    if (otherField != "none") {
      ...
      options.facet = {data: data, x: otherField, transform: d => {console.log(d); return l(d)}}
...
function plotChoropleth(spatialData) {
    ...
    const indexField = otherOptions.geoLevel == "county" ? "county_fips" : "state_fips"
    ...
    const color = {scheme: "rdylbu", type: "diverging", pivot: meanValue, symmetric: true, reverse: true, legend: "ramp", label: otherOptions.measureField}
    ...
    if (otherOptions.geoLevel == "county") {
        ...
        marks.push(Plot.geo(countyGeo, {
            stroke: d => spatialDataMap.get(d.id)?.[otherOptions.measureField], 
            strokeWidth: 0.5,
            fill: d => spatialDataMap.get(d.id)?.[otherOptions.measureField],
        }))
        marks.push(Plot.geo(stateGeo, {stroke: "slategrey", strokeWidth: 1, fill: "none"}))
    } else { 
        marks.push(Plot.geo(stateGeo, {
            stroke: "black",
            strokeWidth: 1,
            fill: d => spatialDataMap.get(d.id)?.[otherOptions.measureField],
        }))
    ...
    geoSelect.on("mouseover", (e,d) => {
        ...
        tooltip.show(mainGeo.features[d].properties.name, centroid[0], centroid[1])
    })
    ...
    d3.select(plotSelect.selectAll("g[aria-label='geo'").nodes()[0])
        .on("mouseleave", (e,d) => {
            plotDemographic(demographicData)
            plotHistogram(spatialData)
        })

    geoSelect.on("mouseover.stroke", (e,d) => {
        d3.select(e.target)
            .attr("stroke", "green")
            .attr("stroke-width", 2)
            .raise()
        plotHistogram(spatialData, [spatialDataMap.get(mainGeo.features[d].id)])
        plotDemographic(demographicData, getDemographicData(data, dataOptionsState,mainGeo.features[d].id))
    })
    .on("mouseleave.stroke", (e,d) => {
        d3.select(e.target)
            .attr("stroke", d => spatialDataMap.get(d.id)?.[otherOptions.measureField])
            .attr("stroke-width", .5)
            .raise()
    })
...
function plotHistogram(data, highlight=[]) {
    ...
    const plot = Plot.plot({
        width: 340,
        height: 260,
        color: {scheme: "rdylbu", type: "diverging", pivot: meanValue, symmetric: true, reverse: true},
        marginBottom: 40,
        x: {label: otherOptions.measureField + " →", tickFormat: tickFormat},
        y: {label: null},
        marks: [
            Plot.rectY(data, Plot.binX({y: "count" }, {x: otherOptions.measureField,  stroke: null, fill: "lightblue"})),
            Plot.ruleX(highlight, {x: d => d[otherOptions.measureField], stroke: "red", strokeWidth: 2}),
            Plot.text(highlight, {x: d => d[otherOptions.measureField],  text: d => d[otherOptions.measureField].toFixed(0), textAnchor: "start", dx:3}),
            //Plot.ruleY([0])
        ]
    })
...
function dataLoaded(loadedData) {
    ...
    const icd10Map = new Map([["All", "All"],  ...causeDict.map(row => [row.code, row.name])])
    ...
    const levelSelect = configureSelect("#levelSelect", LEVELS, d => otherOptions.geoLevel = d)
