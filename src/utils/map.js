...
function plotMapGrid(data, rowField, columnField) {
  ...
  mapsContainer.style.gridTemplateRows = `repeat(${
    rowField ? nRows + 1 : nRows
  }, auto)`;
  mapsContainer.style.gridTemplateColumns = `repeat(${
    columnField ? nColumns + 1 : nColumns
  }, auto)`;
  ...
  const baseHistogramConfig = {
    ...
    marks: [
      Plot.rectY(
        ...
        Plot.binX(
          { y: "count" },
          { x: state.measure, thresholds: 16, fill: "#c3d1c0" }
        )
      ),
    ],
  };
  ...
  if (state.selectCounty != "all") {
    ...
    mainFeatureCollection = {
      type: "FeatureCollection",
      features: state.featureCollection.features.filter(
        (d) => d.id.slice(0, 2) == state.selectState
      ),
    };
    overlayFeatureCollection = {
      type: "FeatureCollection",
      features: state.stateGeo.features.filter(
        (d) => d.id == state.selectState
      ),
    };
  }
...
function addIndividualDownloadButton(element, config) {
  ...
  buttonElement
    .querySelector("#download-data-csv")
...
function addGroupDownloadButton(element, config) {
  ...
  buttonElement
    .querySelector("#download-plot-svg")
    .addEventListener("click", () => {
...
function queryData(stateFips, countyFips) {
  ...
  SELECTABLE_FIELDS.forEach((field) => {
    if (stratifySet.has(field)) {
      mapData = mapData.filter((row) => row[field] != "All");
    } else {
...
function addChoroplethInteractivity(
  ...
  geoSelect
    .on("mouseover.interact", (e, d) => {
      ...
      ${state.conceptMappings.states[feature.id.slice(0, 2)].short}</b>${row[
        state.measure
      ].toFixed(2)}`;
      ...
      const otherRows = state.mapDataGeoMap
        .get(feature.id)
        .filter((d) => d != row);
      ...
      histogramOptions.marks.push(
        Plot.ruleX([row[state.measure]], { stroke: "red", strokeWidth: 1.5 })
      );
...
function updateMapTitle() {
  ...
  const mapTitleWrapper = document.createElement("div");
  ...
  console.log({mapSelectionElements});
