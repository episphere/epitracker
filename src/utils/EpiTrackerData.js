...
export class EpiTrackerData {
  ...
  async getQuantileMortalityData(query, args) {
    ...
    let quantileMortalityData = (await this.#loadQuantileMortalityData(year))
      .rename( {
        race_ethnicity: "race",
      })
    ...
    return this.postProcessCountyMortalityData(quantileMortalityData
      .filter(filterString)
      .derive({
  ...
  async #loadDemographicData(year) {
    ...
    return this.#smartLoadZipData(`../data/demographic/demographic_data_${year}.csv.zip`, `demographic_data_${year}.csv`)
  }
  ...
  async #loadCountyMortalityData(year) {
    ...
    return this.#smartLoadZipData(`../data/mortality/age_adjusted_data_${year}.csv.zip`, `age_adjusted_data_${year}.csv`)
  }
  ...
  async #loadQuantileMortalityData(year) {
    ...
    return this.#smartLoadZipData(`../data/quantile/quantile_data_${year}.csv.zip`, `quantile_data_${year}.csv`)
  }
